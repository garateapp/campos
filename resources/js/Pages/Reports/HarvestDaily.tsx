...
export default function HarvestDaily({ rows, fields, filters }: Props) {
    ...
    const totalKg = rows.reduce((sum, row) => sum + Number(row.total_kg || 0), 0);
                            ...
                            <div className="flex flex-wrap gap-6 text-sm text-gray-600">
                                <div>
                                    <span className="font-semibold text-gray-900">
                                        {totalKg.toLocaleString('es-CL', { maximumFractionDigits: 2 })}
                                    </span>{' '}
                                    kg totales
                                </div>
                                <div>
                                    <span className="font-semibold text-gray-900">
                                        ${totalValue.toLocaleString('es-CL', { maximumFractionDigits: 0 })}
                                    </span>{' '}
                                    total estimado
                                </div>
                            </div>
                            ...
                            <table className="min-w-full divide-y divide-gray-200">
                                ...
                                <tbody className="bg-white divide-y divide-gray-200">
                                    {rows.length === 0 ? (
                                        ...
                                        rows.map((row, index) => (
                                            <tr key={`${row.harvest_date}-${row.field_id}-${index}`} className="hover:bg-gray-50 transition-colors">
                                                ...
                                                <td className="px-6 py-4 whitespace-nowrap text-sm text-right text-green-700 font-semibold">
                                                    {Number(row.total_kg).toLocaleString('es-CL', { maximumFractionDigits: 2 })} kg
                                                </td>
                                                <td className="px-6 py-4 whitespace-nowrap text-sm text-right text-gray-700">
                                                    ${Number(row.total_value).toLocaleString('es-CL', { maximumFractionDigits: 0 })}
                                                </td>
                                            </tr>
                                        ))
                                    )}
