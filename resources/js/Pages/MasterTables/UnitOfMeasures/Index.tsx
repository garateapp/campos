...
export default function Index({ units }: IndexProps) {
    ...
    const submit: FormEventHandler = (e) => {
        e.preventDefault();
        if (editingUnit) {
            patch(route('unit-of-measures.update', editingUnit.id), {
                onSuccess: () => closeModal(),
            });
        } else {
            post(route('unit-of-measures.store'), {
                onSuccess: () => closeModal(),
            });
        }
    };
    ...
    const handleDelete = (unit: UnitOfMeasure) => {
        if (confirm(`¿Estás seguro de eliminar la unidad "${unit.name}"?`)) {
            router.delete(route('unit-of-measures.destroy', unit.id));
        }
    };
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-gray-500">
                                Unidades estandarizadas para tus labores (Kg, Ha, Día, etc.)
                            </p>
                        </div>
                    </div>
                    <button
                                    ...
                                    units.map((unit) => (
                                        <tr key={unit.id} className="hover:bg-gray-50 transition-colors">
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                                {unit.code ? <span className="bg-gray-100 px-2 py-1 rounded text-xs font-mono">{unit.code}</span> : '-'}
                                            </td>
                ...
                <form onSubmit={submit} className="p-6">
                    ...
                    <div className="mb-4">
                        ...
                        <TextInput
                            ...
                            required
                            isFocused
                            placeholder="Ej: Kilogramos, Hectáreas..."
                        />
                        <InputError message={errors.name} className="mt-2" />
                    </div>

                    <div>
                        <InputLabel htmlFor="code" value="Código Corto" />
                        <TextInput
                            id="code"
                            type="text"
                            name="code"
